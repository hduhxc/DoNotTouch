...
var canvasRatio = 0.6; 			//游戏画面占屏幕的比例
...
var Container = function(correctPos, correctType, posY, itemState) {
	...
	this.itemState   = itemState;   //每一格的点击状态
	this.correctPos  = correctPos;  //正确的格子位置
	this.correctType = correctType; //正确的格子类型
};
var imgsUntouch   = new Array(); //含每种类型的格子(未点击）的image对象
var imgsTouched   = new Array(); //含每种类型的格子(已点击）的image对象
var imgError      = new Image(); //发生错误(点击到白格)时的image对象
...
function startGame() {
	...
	$('#gameCanvas').unbind('click').click(function(event) {
		//获取相对于Canvas的坐标
...
function gameOverPage() {
	...
	isPlaying = false;
	$('#playAgainBtn').unbind('click').click(function() {
		$('#end').hide();
		$('#game').fadeIn('fast').show();

		startGame();
	});
	$('#returnBtn').unbind('click').click(function() {
		$('#end').hide();
...
function selectSkinPage() {
	...
	$('#selectSkinBtn').unbind('click').click(function() {
...
$(document).ready(function() {
	...
	$('#endlessStartBtn').click(function() {
		$('#start').hide();
		$('#game').fadeIn('fast').show();
